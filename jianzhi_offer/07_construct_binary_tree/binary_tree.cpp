/*
 * 面试题7：重建二叉树
 * 题目：输入某二叉树的前序遍历和中序遍历的结果，请重建出该二叉树。假设输入的前序遍历
 * 和中序遍历的结果中都不含重复的数字。例如输入前序遍历序列{1,2, 4, 7, 3, 5, 6, 8}
 * 和中序遍历序列{4, 7, 2, 1, 5, 3, 8, 6}，则重建出图2.6所示的二叉树并输出它的头结点。
 */
#include <cstdio>
#include <stdexcept>
using namespace std;

/*
 * 二叉树结点
 */
struct BinaryTreeNode
{
  int value;
  BinaryTreeNode* pLeft;
  BinaryTreeNode* pRight;
};

// 打印二叉树
void print_tree(BinaryTreeNode* root){
  if(root == nullptr)
    return;

  printf("%d ", root->value);
  
  print_tree(root->pLeft);
  print_tree(root->pRight);
}

// 删除二叉树
void destroy_tree(BinaryTreeNode* root){
  if(root != nullptr){
    BinaryTreeNode* pLeft = root->pLeft;
    BinaryTreeNode* pRight = root->pRight;

    delete root;
    root = nullptr;

    destroy_tree(pLeft);
    destroy_tree(pRight);
  }
}

/*
 * 参数：
 *  preorder 前序遍历数组
 *  inorder 中序遍历数组
 *  length 数组长度，即数结点个数
 * 返回值：
 *  构建的二叉树的根结点指针
 * 复杂度：
 *  时间：O(nlogn)
 *  空间：O(1)
 */
BinaryTreeNode* construct_tree(int* preorder, int* inorder, int length){
  // 判断输入有效
  if(preorder == nullptr || inorder == nullptr || length <= 0)
    throw runtime_error("input invalid.");

  // 算法：1.从前序数组中取出第一个数字，创建根结点
  BinaryTreeNode* root = new BinaryTreeNode();
  root->value = *preorder;
  root->pLeft = root->pRight = nullptr;

  // 递归终止条件：只有一个结点
  if(length == 1){
    if(*preorder == *inorder){
      return root;
    }else{
      throw runtime_error("input invalid.");
    }
  }

  // 算法：2.从中序数组中找到根结点
  int *inorderRoot = inorder;
  while(inorderRoot < inorder+length){
    if(*inorderRoot == root->value)
      break;
    ++inorderRoot;
  }

  if(*inorderRoot != root->value)
    throw runtime_error("input invalid.");
  
  // 算法：3.在中序和前序数组中分别划分左右子树
  int leftLength = inorderRoot - inorder;
  if(leftLength > 0){
    root->pLeft = construct_tree(preorder+1, inorder, leftLength);
  }
  
  int rightLength = length - leftLength - 1;
  if(rightLength > 0){
    root->pRight = construct_tree(preorder+leftLength+1, inorderRoot+1, rightLength);
  }

  return root;
}

/*
 * 测试代码
 */
void Test(const char* testName, int* preorder, int* inorder, int length){
  if(testName != nullptr)
    printf("%s begins:\n", testName);
  
  printf("The preorder sequence is: ");
  for(int i=0; i<length; i++){
    printf("%d ", preorder[i]);
  }
  printf("\n");

  printf("The inorder  sequence is: ");
  for(int i=0; i<length; i++){
    printf("%d ", inorder[i]);
  }
  printf("\n");

  try{
    BinaryTreeNode* root = construct_tree(preorder, inorder, length);
    print_tree(root);
    printf("\n");
    
    destroy_tree(root);
  }catch(exception& e){
    printf("%s", e.what());
    printf("\n");
  }
}

/* 测试用例
// 普通二叉树
//              1
//           /     \
//          2       3  
//         /       / \
//        4       5   6
//         \         /
//          7       8
*/
void Test1()
{
    const int length = 8;
    int preorder[length] = {1, 2, 4, 7, 3, 5, 6, 8};
    int inorder[length] = {4, 7, 2, 1, 5, 3, 8, 6};

    Test("Test1", preorder, inorder, length);
}

/* 所有结点都没有右子结点
//            1
//           / 
//          2   
//         / 
//        3 
//       /
//      4
//     /
//    5
*/
void Test2()
{
    const int length = 5;
    int preorder[length] = {1, 2, 3, 4, 5};
    int inorder[length] = {5, 4, 3, 2, 1};

    Test("Test2", preorder, inorder, length);
}

/* 所有结点都没有左子结点
//            1
//             \ 
//              2   
//               \ 
//                3 
//                 \
//                  4
//                   \
//                    5
*/
void Test3()
{
    const int length = 5;
    int preorder[length] = {1, 2, 3, 4, 5};
    int inorder[length] = {1, 2, 3, 4, 5};

    Test("Test3", preorder, inorder, length);
}

// 树中只有一个结点
void Test4()
{
    const int length = 1;
    int preorder[length] = {1};
    int inorder[length] = {1};

    Test("Test4", preorder, inorder, length);
}

/* 完全二叉树
//              1
//           /     \
//          2       3  
//         / \     / \
//        4   5   6   7
*/
void Test5()
{
    const int length = 7;
    int preorder[length] = {1, 2, 4, 5, 3, 6, 7};
    int inorder[length] = {4, 2, 5, 1, 6, 3, 7};

    Test("Test5", preorder, inorder, length);
}

// 输入空指针
void Test6()
{
    Test("Test6", nullptr, nullptr, 0);
}

// 输入的两个序列不匹配
void Test7()
{
    const int length = 7;
    int preorder[length] = {1, 2, 4, 5, 3, 6, 7};
    int inorder[length] = {4, 2, 8, 1, 6, 3, 7};

    Test("Test7: for unmatched input", preorder, inorder, length);
}

int main(int argc, char* argv[])
{
    Test1();
    Test2();
    Test3();
    Test4();
    Test5();
    Test6();
    Test7();

    return 0;
}
